      ...
      switch (answer.appChoice) {
        case "spotify":
          inquirer.prompt(appQuestions.spotifyArtistOrTrackQuestion).then(function(answer) {
              ...
              else if (answer.spotifyArtistOrTrackChoice.toLowerCase() == "track") {
                inquirer.prompt(appQuestions.spotifyTrackQuestion).then(function(answer) {
                  getSpotify("track", answer.trackChoice);
                });
              }
              else {console.log(chalk.yellow("ERROR ERROR ERROR"))}
            });
            break;
            ...
            case "twitter":
              inquirer.prompt(appQuestions.twitterQuestion).then(function(answer) {
                params = {
                  screen_name: answer.twitterChoice,
                  count: 5,
                  include_rts: 0
                };
                getTweets(params);
